...
int main(int argc, char *argv[])
{
    ...
    {
        router.set(zmqpp::socket_option::plain_username,FLAGS_user_name);
        router.set(zmqpp::socket_option::plain_password,FLAGS_password);
    }
    ...
    if (FLAGS_endpoint.find(",")!=std::string::npos)
    {
        ...
        router.connect(FLAGS_endpoint);
    }

    zmqpp::loop looper;
    looper.add(std::chrono::milliseconds(1000),0,std::bind(send_msg,std::ref(router)));
    looper.start();
...
static unsigned long flag_index=0;
bool send_msg(zmqpp::socket &socket)
{
    ...
    zmqpp::message msg;
    msg.add(flag_index++%2==0?"server1":"server2");
    msg.add(ss.str());
